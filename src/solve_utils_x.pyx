...
def cell_val_has_no_conflicts(v, grid, r, c):
    ...
    for r1 in range(9):
        for c1 in range(9):
            G[r1][c1] = grid[r1][c1]
    v1 = v; r1 = r; c1 = c
    return cell_val_has_no_conflicts_c(v1, G, r1, c1)
...
def determine_cands(Grid, Elims = None):
    ...
    cdef int CyGrid[9][9]
    cdef bint CyElims[9][9][9]
    cdef bint CyCands[9][9][9]
    ...
    if Elims:
        for r in range(9):
            for c in range(9):
                CyGrid[r][c] = Grid[r][c]
                for d in range(9):
                    CyElims[r][c][d] = CyCands[r][c][d] = False
                for Cand in Elims[r][c]:
                    CyElims[r][c][Cand-1] = True
    else:
        for r in range(9):
            for c in range(9):
                CyGrid[r][c] = Grid[r][c]
                for d in range(9):
                    CyElims[r][c][d] = CyCands[r][c][d] = False
    NrEmpties = determine_cands_c(CyGrid, CyElims, CyCands)
    Cands = [[set() for c in range(9)] for r in range(9)]
    for r in range(9):
        for c in range(9):
            for d in range(9):
                if CyCands[r][c][d]: Cands[r][c].add(d+1)
