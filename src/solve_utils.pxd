...
ctypedef struct TREE_FFISH:  # FFISH ==> finned fish trees, one for each cover in a linked list
    ...
    int             nFins
    # int             FinState[3]  # SEARCHING = 0, FOUND, NOT_FOUND, BOTTOMED_OUT (recursion limit)
    NODE            *FinChain[3]  # Points to the fin chains (cvr first to fin last) when a valid pattern (chain from each fin to cover) is found
    TNODE_FFISH     *FinBranch[3]  # Tree has a first level branch for each fin..
    TREE_FFISH      *Prev
    TREE_FFISH      *Next
