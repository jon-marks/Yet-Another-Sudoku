        ...
        for r0 in range(8):
            ...
            for r1 in range(r0+1, 9):
                ...
                for cs in CU:
                    ...
                    if _elim_cands_in_fish(Cand, BS, CU, P_ROW, Cands, ElimCands, Step) == 0:
                        return 0
        # look at cols
        BC = [set() for i in range(9)]
        for c in range(9):
        ...
        for c0 in range(8):
            ...
            for c1 in range(c0+1, 9):
                ...
                for cs in CU:
                    ...
                    if _elim_cands_in_fish(Cand, BS, CU, P_COL, Cands, ElimCands, Step) == 0:
                        return 0
    return -1
...
#                                     SubStep = {P_TECH: T_UNDEF, P_PTRN: [], P_OUTC: [], P_SUBS: []}
#                                     if _elim_cands_in_fish(Cand, BS, CS1, P_ROW, Cands, ElimCands, SubStep) == 0:
#                                         SubStep[P_PTRN] = [[P_VAL, Cand], [P_OP, OP_POS],
#                                                            [P_ROW, r4], [P_COL, c4],
#                                                            [P_CON, ]]+SubStep[P_PTRN]
...
#                         BR = sorted(BS)
#                         Step[P_PTRN] = [[P_VAL, Cand], [P_ROW, BR], [P_COL, CS]]
#                         for br in BR:
#                             for fc in sorted(CF):
#                                 if Cand in Cands[br][fc]:
#                                     Step[P_PTRN].extend([[P_CON, ], [P_ROW, br], [P_COL, fc]])
...
#                         Step[P_TECH] = T_ALMOST_X_WING
#                         BC = sorted(BS)
#                         Step[P_PTRN] = [[P_VAL, Cand], [P_COL, BC], [P_ROW, CS]]
#                         for bc in BC:
#                             for fr in sorted(CF):
#                                 if Cand in Cands[fr][bc]:
#                                     Step[P_PTRN].extend([[P_CON, ], [P_ROW, fr], [P_COL, bc]])
#                         Step[P_PTRN].append([P_END, ])
#                         return 0
#     return -1
...
#                             Step[P_TECH] = T_ALMOST_SWORDFISH
#                             BC = sorted(BS)
#                             Step[P_PTRN] = [[P_COL, BC], [P_ROW, CS]]
#                             for bc in BC:
#                                 for fr in sorted(CF):
#                                     if Cand in Cands[fr][bc]:
#                                         Step[P_PTRN].extend([[P_CON, ], [P_ROW, fr], [P_COL, bc]])
#                             Step[P_PTRN].extend([[P_OP, OP_PRES], [P_VAL, Cand], [P_END, ]])
#                             return 0
#     return -1
...
def _elim_cands_in_finned_fish(Cand, BS, CS, CF, rc, Cands, ElimCands, Step, AIC = 0):
    ...
    res = _elim_cands_in_finned_fish1(Cand, BS, CS, CF, rc, Cands, ElimCands, Step, AIC = AIC)
    if res == -1 and AIC:
        return _elim_cands_in_finned_fish1(Cand, BS, CS, CF, rc, Cands, ElimCands, Step, AIC = AIC, GrpLks = True)
    return res

def _elim_cands_in_finned_fish1(Cand, BS, CS, CF, rc, Cands, ElimCands, Step, AIC = 0, GrpLks = False):
    Fins = []
    Chains = []
    AICFound = False
    ...
    if Step[P_OUTC]:
        ...
        Step[P_DIFF] = T[Step[P_TECH]][T_DIFF] + NrLks * AIC_LK_DIFF + NrGrpLks * AIC_GRP_LK_DIFF
        if rc == P_ROW:
