                ...
                for (N2, N3) in Lks[i]:
                    ...
                    for Nx in Lx:
                        ...
                        if Lk != LK_NONE:
                            ...
                            X1.UN.extend(Lx)
                            XCuc1.append(X1)
            if not XCuc1: CandsXC.discard(Cand)
            XCuc[i] = XCuc1
...
def find_x_chain_starts(Cands, GrpLks = False):
    ...
    for i, Cand in enumerate(range(1, 10)):
        Lks[i] = find_strong_single_cand_links_btwn_cells(Cand, Cands, GrpLks)
...
def find_strong_x_loop_starts(Cands, GrpLks = False):
    ...
    for i, Cand in enumerate(range(1, 10)):
        Lks[i] = find_strong_single_cand_links_btwn_cells(Cand, Cands, GrpLks)
        # Note lks [i] ==> ((r, c), (r, c))  # Cand is assumed known because it was passed
        lenLks = len(Lks[i])
        for l0 in range(lenLks-1):
            N0a, N0b = Lks[i][l0]
            for l1 in range(l0+1, lenLks):
