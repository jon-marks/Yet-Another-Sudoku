.. title:: Yet Another Sudoku | Human Solvable Patterns

.. include:: ..\globals.inc

.. _human-solvable-patterns:

=======================
Human Solvable Patterns
=======================

Human solvable patterns rely on logic that is not too complicated and involved to resolve.  As
expertise in solving puzzles improves, one can resolve more complicated patterns

Practically, a point of diminishing returns exists between:
   *  the complexity of the pattern,
   *  the complexity of the logic to resolve the pattern, and
   *  the probability of the pattern’s occurrence.

This text is not an exhaustive coverage of all possible conditional logic techniques.
The decision of what or not to include is subjective especially when it comes to those esoteric and
convoluted techniques involving a mix of other complex techniques.

Many patterns can be identified and resolved by more than one logic technique.  Where multiple
techniques can resolve a pattern, the technique with the broadest reach is favoured, unless it is
much more complex than another technique.

Many resources on the internet exist describing the identification and resolution of patterns
effectively. Rather than rehashing their information in this text, I, with gratitude, link to their
descriptions.

.. toctree::
   :maxdepth: 5

   01-singles
   02-subsets
   03-bent-subsets
   04-fish
   05-chains-loops-nets
   06-unlocked-sets
   07-uniqueness
   08-searching
   09-super-human
   10-last-resort
